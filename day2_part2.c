...
void find_nums(int arr[], int arr_len);
...
#define TRY_SIZE    100
    for (i = 0; i < 100; i++)
    {
        for (j = 0; j < 100; j++)
        {
            memset(arr_cpy, 0, arr_len * sizeof(int));
            memcpy(arr_cpy, arr, arr_len * sizeof(int));
            arr_cpy[1] = i;
            arr_cpy[2] = j;
            parse_arr(arr_cpy, arr_len);
#define MAGIC_NUM   19690720
            printf("[*] Trying noun: %d, verb: %d, result: %d\n",i,j, arr_cpy[0]);
            if (arr_cpy[0] == MAGIC_NUM)
            {
                printf("[*] FOUND IT: noun: %d, verb: %d, result: %d\n",i,j, arr_cpy[0]);
                return;
            }
        }
        print_arr(arr_cpy, arr_len);
    }
}
    ...
    for (i = 0; i < arr_len; i++)
    {
        ...
        switch (instr)
        {
            ...
            case 99:
                ...
                break;
            default:
//                printf("[*] unexpected opcode: %d at address: %d\n",instr, i);
                break;
        }
    }
    printf("\n");
